...
export default function MarketDashboard({ allStockQuotes, news }) {
  ...
  // Map index quotes to display data
  ...
  const topMovers = useMemo(() => {
    ...
    // Take top 6 gainers and top 5 losers for variety
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, padding: 10 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 0 }}>
            {indices.map((idx, i) => (
              <div
                ...
                style={{
                  padding: "10px 12px",
                  borderRight: i < 9 ? `1px solid ${COLORS.border}22` : "none",
                  borderBottom: i < 5 ? `1px solid ${COLORS.border}22` : "none",
                }}
              >
                ...
                <div style={{ fontSize: 14, fontWeight: 700, color: COLORS.text, fontFamily: "'JetBrains Mono',monospace" }}>
      ...
      <Panel>
        <PanelHeader
          icon={<BarChart3 size={14} color={COLORS.orange} />}
          title="TOP MOVERS"
          subtitle="Biggest price changes today"
        />
        <div style={{ padding: 8 }}>
          {topMovers.length === 0 ? (
              ...
              >
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <div style={{ width: 60, height: 6, background: COLORS.bgPanel, borderRadius: 3, overflow: "hidden" }}>
                    <div
                      style={{
                        width: `${Math.min(Math.abs(s.val) * 15, 100)}%`,
                        height: "100%",
                        background: s.val >= 0 ? COLORS.green : COLORS.red,
                        borderRadius: 3,
                        marginLeft: s.val < 0 ? "auto" : 0,
                      }}
                    />
                  </div>
                  <ChgVal val={s.val} />
          ...
          {newsItems.length === 0 ? (
            ...
            newsItems.map((n, i) => (
              <a
                ...
                style={{ textDecoration: "none", display: "block", padding: "6px 10px", borderBottom: `1px solid ${COLORS.border}22`, cursor: "pointer" }}
      ...
      <Panel>
        <PanelHeader
          icon={<Layers size={14} color={COLORS.cyan} />}
          title="MARKET MAP"
          subtitle="Top stocks by market cap"
        />
        <div style={{ padding: 8 }}>
              ...
              {treemapData.map((t) => (
                <div
                  ...
                  style={{
                    ...
                    background:
                      t.chg >= 0
                        ? `rgba(34,197,94,${Math.min(Math.abs(t.chg) * 0.15, 0.5)})`
                        : `rgba(239,68,68,${Math.min(Math.abs(t.chg) * 0.15, 0.5)})`,
                    border: `1px solid ${t.chg >= 0 ? COLORS.green + "33" : COLORS.red + "33"}`,
                  }}
                >
                  ...
                  <div
                    style={{
                      fontSize: 9,
                      color: t.chg >= 0 ? COLORS.green : COLORS.red,
      ...
      <Panel style={{ gridColumn: "1/3" }}>
        <PanelHeader
          icon={<Hash size={14} color={COLORS.gold} />}
          title="COMMODITIES"
          subtitle="Real-time commodity prices"
        />
        <MiniTable
