      ...
      <div style={{ borderRight: `1px solid ${COLORS.border}`, background: COLORS.bgPanel, overflowY: "auto" }}>
        ...
        {loading ? (
          ...
          fxPairs.map((p) => (
            <div
              key={p.symbol}
              onClick={() => setSelPairSymbol(p.symbol)}
              style={{
                padding: "8px 12px",
                cursor: "pointer",
                borderBottom: `1px solid ${COLORS.border}22`,
                background: selPairSymbol === p.symbol ? COLORS.purpleDim + "44" : "transparent",
                borderLeft: selPairSymbol === p.symbol ? `3px solid ${COLORS.purple}` : "3px solid transparent",
              }}
            >
              ...
              <div style={{ display: "flex", justifyContent: "space-between", marginTop: 2 }}>
                <span style={{ fontSize: 11, color: COLORS.green, fontFamily: "'JetBrains Mono',monospace" }}>
                  {fmt(p.price, 4)}
                </span>
                <span style={{ fontSize: 9, color: COLORS.textMuted }}>
                  {p.change >= 0 ? "+" : ""}{fmt(p.change, 4)}
                </span>
              </div>
            </div>
          ))
        ...
        <div style={{ padding: 12, display: "grid", gridTemplateColumns: "1fr 300px", gap: 10 }}>
          <Panel>
            <PanelHeader
              ...
              title={`${selConfig.pair} — 90 DAY CHART`}
              subtitle="Historical exchange rate"
            />
              ...
              <div style={{ padding: 12 }}>
                <div style={{ marginBottom: 8 }}>
                  <div style={{ fontSize: 10, color: COLORS.textMuted, marginBottom: 4 }}>
                    Amount ({selConfig.pair.split("/")[0]})
                  </div>
                  <input
                    value={calcFrom}
                    onChange={(e) => setCalcFrom(e.target.value)}
                    style={{
                      width: "100%", padding: 8, background: COLORS.bgInput,
                ...
                <div style={{ fontSize: 10, color: COLORS.textMuted, marginBottom: 4 }}>
                  Converted ({selConfig.pair.split("/")[1]})
                </div>
                <div
                  style={{
                    padding: 8, background: COLORS.purpleDim + "33", borderRadius: 4,
                ...
                <div style={{ marginTop: 8, fontSize: 10, color: COLORS.textDim }}>
                  Rate: 1 {selConfig.pair.split("/")[0]} = {fmt(bid, 4)} {selConfig.pair.split("/")[1]}
                </div>
              </div>
            </Panel>
