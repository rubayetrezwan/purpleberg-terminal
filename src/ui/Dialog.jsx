...
// Singleton: the fixed ids are safe because only one DialogHost is ever mounted.
export function DialogHost() {
  ...
  return (
    <div className="pb-scrim pb-scrim--center" onMouseDown={(e) => { if (e.target === e.currentTarget) settleDialog(false); }}>
      <div ref={ref} className="pb-dialogbox" role="alertdialog" aria-modal="true" aria-labelledby="pb-dialog-title" aria-describedby={d.body ? "pb-dialog-body" : undefined} tabIndex={-1}>
        <div id="pb-dialog-title" className="pb-dialogbox__title">{d.title}</div>
        {d.body && <div id="pb-dialog-body" className="pb-dialogbox__body">{d.body}</div>}
