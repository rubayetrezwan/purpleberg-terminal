...
test("sparklinePoints scales values into the box, centres flat lines, skips junk", () => {
  ...
  assert.equal(sparklinePoints([3, 2, 1], 60, 14, 1), "1.0,1.0 30.0,7.0 59.0,13.0");
});
...
test("freshnessState labels and staleness", () => {
  ...
  assert.deepEqual(freshnessState(0, 9000, 15000, true), { label: "9s ago", tone: "muted", stale: false }); // epoch 0 is a timestamp, not "missing"
});
