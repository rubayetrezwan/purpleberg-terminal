...
// ── GET /api/dse ────────────────────────────────────────
// Scrape DSE (Dhaka Stock Exchange) share prices
app.get("/api/dse", async (req, res) => {
  try {
    const cacheKey = "dse:all";
    const data = await cached(cacheKey, 60_000, async () => {
      ...
      if (!r.ok) throw new Error(`DSE fetch error: ${r.status}`);
      const html = await r.text();

      // Parse the HTML table - DSE uses a simple table format
      const rows = [];
      ...
      for (const row of matches) {
        ...
        // DSE table format: #, Trading Code, LTP, High, Low, Close Price, YCP, Change, Trade, Value, Volume
        if (cells.length >= 10 && !isNaN(parseFloat(cells[2]))) {
          const ltp = parseFloat(cells[2]) || 0;
          const ycp = parseFloat(cells[6]) || 0;
          ...
          rows.push({
            ...
            high: parseFloat(cells[3]) || 0,
            low: parseFloat(cells[4]) || 0,
            prevClose: ycp,
            volume: parseInt(cells[10]?.replace(/,/g, "")) || 0,
            value: parseFloat(cells[9]?.replace(/,/g, "")) || 0,
            trades: parseInt(cells[8]?.replace(/,/g, "")) || 0,
            exchange: "DSE",
            currency: "BDT",
            marketState: "REGULAR",
            marketCap: 0,
            pe: 0, eps: 0, week52High: 0, week52Low: 0, avgVolume: 0,
            dividendYield: 0, beta: 0, open: ltp,
          });
        }
      }
    ...
    console.error("DSE error:", e.message);
    res.json([]);
  }
});
