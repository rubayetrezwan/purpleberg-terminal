...
// ── GET /api/dse ────────────────────────────────────────
// Scrape real-time DSEX stock data from Dhaka Stock Exchange
app.get("/api/dse", async (req, res) => {
  try {
    const cacheKey = "dse:all";
    const data = await cached(cacheKey, 60_000, async () => {
      const r = await fetch("https://www.dsebd.org/latest_share_price_scroll_l.php", {
        headers: { "User-Agent": UA },
      });
      if (!r.ok) throw new Error(`DSE fetch error: ${r.status}`);
      const html = await r.text();

      // Parse table rows: columns are #, Symbol, LTP, High, Low, Close, YCP, Change, Trade, Value(mn), Volume
      ...
      for (const row of rows) {
        ...
        if (cells.length >= 10 && !isNaN(parseInt(cells[0]))) {
          ...
          const high = parseFloat(cells[3]) || 0;
          const low = parseFloat(cells[4]) || 0;
          const close = parseFloat(cells[5]) || 0;
          const ycp = parseFloat(cells[6]) || 0;
          const change = parseFloat(cells[7]) || 0;
          const volume = parseInt(cells[10]) || 0;
